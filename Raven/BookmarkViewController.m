...
#define LISTVIEW_CELL_IDENTIFIER		@"MyBookmarkCell"
#define LISTVIEW_CELL_IDENTIFIER_history		@"MyListViewCell"
- (void)awakeFromNib
{
...
{
    if ([selectorButton selectedSegment] == 0) {
	MyBookmarkCell *cell = (MyBookmarkCell*)[aListView dequeueCellWithReusableIdentifier:LISTVIEW_CELL_IDENTIFIER];
	
        if(!cell) {
            cell = [MyBookmarkCell cellLoadedFromNibNamed:LISTVIEW_CELL_IDENTIFIER reusableIdentifier:LISTVIEW_CELL_IDENTIFIER];
        }
    ...
    {
        MyListViewCell *cell = (MyListViewCell*)[aListView dequeueCellWithReusableIdentifier:LISTVIEW_CELL_IDENTIFIER_history];
        
        if(!cell) {
            cell = [MyListViewCell cellLoadedFromNibNamed:LISTVIEW_CELL_IDENTIFIER_history reusableIdentifier:LISTVIEW_CELL_IDENTIFIER_history];
        }
        
        // Set up the new cell:
        DatabaseController *controler = [DatabaseController sharedUser];
        //Get the bookmark at the index from the selected row
        bookmarkObject *bookmark = (bookmarkObject *)[controler.bookmarks objectAtIndex:row]; 
...
-(void)listView:(PXListView *)aListView rowDoubleClicked:(NSUInteger)rowIndex
{
     ...
     */
    DatabaseController *controller = [DatabaseController sharedUser];
    bookmarkObject *bookmark = (bookmarkObject *)[controller.bookmarks objectAtIndex:rowIndex]; 
