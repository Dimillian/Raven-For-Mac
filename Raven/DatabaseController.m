...
static DatabaseController *sharedUserManager = nil;

+(DatabaseController *)sharedUser
{
	...
	if(sqlite3_open([databasePath UTF8String], &database) == SQLITE_OK) {
		...
		if(sqlite3_prepare_v2(database, sqlStatement, -1, &compiledStatement, NULL) == SQLITE_OK) {
			...
			while(sqlite3_step(compiledStatement) == SQLITE_ROW) {
				...
				bookmarkObject *bookmark = [[bookmarkObject alloc] initBookmarkAndFavoriteWithName:aTitle 
                                                                                               url:aUrl 
                                                                                            favico:image 
                                                                                              udid:udid 
                                                                                              type:type
	...
	if(sqlite3_open([databasePath UTF8String], &database) == SQLITE_OK) {
		...
		if(sqlite3_prepare_v2(database, sqlStatement, -1, &compiledStatement, NULL) == SQLITE_OK) {
			...
			while(sqlite3_step(compiledStatement) == SQLITE_ROW) {
				...
				bookmarkObject *historyItem = [[bookmarkObject alloc]initHistoryInitWithName:aTitle 
                                                                                      url:aUrl 
                                                                                   favico:image 
                                                                                     date:aDate
                                                                                        udid:udid]; 
	...
	if(sqlite3_open([databasePath UTF8String], &database) == SQLITE_OK) {
		...
		if(sqlite3_prepare_v2(database, sqlStatement, -1, &compiledStatement, NULL) == SQLITE_OK) {
			...
			while(sqlite3_step(compiledStatement) == SQLITE_ROW) {
                ...
                [image setSize:NSMakeSize(16, 16)]; 
                //Create a bookmark object with the database lavue
				bookmarkObject *aSuggestion = [[bookmarkObject alloc] initWithName:aTitle url:aUrl favico:image udid:udid]; 
